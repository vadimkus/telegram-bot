...
const tmdbScraper = require('../lib/simple-tmdb');
...
Use /start to select your favorite genre, then come back here for personalized recommendations!`;
    ...
    if (contentType === 'series') {
      newReleases = await tmdbScraper.getPopularTVSeries();
    } else {
      newReleases = await tmdbScraper.getPopularMovies();
    }
...
bot.action('top_rated_movies', async (ctx) => {
  try {
    ...
    const movies = await tmdbScraper.getTopRatedMovies();
...
bot.action('now_playing_movies', async (ctx) => {
  try {
    ...
    const movies = await tmdbScraper.getNowPlayingMovies();
...
bot.action('top_rated_series', async (ctx) => {
  try {
    ...
    const series = await tmdbScraper.getTopRatedTVSeries();
...
bot.action('now_airing_series', async (ctx) => {
  try {
    ...
    const series = await tmdbScraper.getNowAiringTVSeries();
